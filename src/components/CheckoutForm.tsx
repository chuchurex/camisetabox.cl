...
export default function CheckoutForm({ productId, productName, productPrice }: CheckoutFormProps) {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'flow' | 'paypal' | 'stripe'>('flow');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (paymentMethod === 'flow' && data.url) {
        window.location.href = data.url;
      } else if (paymentMethod === 'paypal' && data.approvalUrl) {
        window.location.href = data.approvalUrl;
      } else if (paymentMethod === 'stripe' && data.url) {
        window.location.href = data.url;
      }
    } catch (err: any) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <button
            ...
            className={`p-4 rounded-lg border-2 transition-all ${
              paymentMethod === 'flow'
                ...
                : 'border-gray-700 hover:border-[#00d2ff]/50'
            }`}
          >
            <div className="flex flex-col items-center gap-2">
              <div className="text-2xl font-bold text-[#00d2ff]">Flow</div>
              <span className="text-xs text-gray-400">Pagos Chile</span>
            </div>
          </button>
          ...
          >
            <div className="flex flex-col items-center gap-2">
              <svg className="w-16 h-6" viewBox="0 0 124 33" fill="currentColor">
