...
export default function CheckoutForm({ productId, productName, productPrice }: CheckoutFormProps) {
  ...
  const paymentMethod = 'paypal'; // Solo PayPal disponible por ahora
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.approvalUrl) {
        ...
        throw new Error('No se recibió URL de pago de PayPal');
      }
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Payment Method - Solo PayPal */}
      <div>
        <label className="block text-sm font-semibold mb-3">
          Método de pago
        </label>
        <div className="p-6 rounded-lg border-2 border-[#0070ba] bg-[#0070ba]/5">
          <div className="flex items-center justify-center gap-3">
            <svg className="w-24 h-8" viewBox="0 0 124 33" fill="currentColor">
            ...
            <div className="text-left">
              <div className="text-sm font-semibold text-white">Pago seguro con PayPal</div>
              <div className="text-xs text-gray-400">Acepta tarjetas de crédito/débito y cuentas PayPal</div>
            </div>
          </div>
        </div>
      </div>
